...
private:
    ...
    int VIOLATES(RedBlackTreeNode *node, int bh = 0) {
        if(node == NIL) {
            blackDepth.insert(bh);
            if(blackDepth.size() > 1) {
                return 4;
            }
        ...
        int x = VIOLATES(node -> left, bh);

        if(x != 0) {
            return x;
        }

        return VIOLATES(node -> right, bh);
    }
